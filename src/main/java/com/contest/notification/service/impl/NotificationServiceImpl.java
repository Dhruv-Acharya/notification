...
public class NotificationServiceImpl implements NotificationService {
    ...
    @Override
    public List<NotificationHistoryDTO> findByUserIdPagenation(String userId,int pageLimit) throws Exception {
        int size = notificationRepository.countByReceiver(userId);

        List<NotificationData> page= notificationRepository.findByReceiver(userId,new PageRequest((size/10)-pageLimit,10,Sort.Direction.DESC,"TimeStamp"));
        List<NotificationHistoryDTO> notificationHistoryDTOList = new ArrayList<>();
        for ( NotificationData singleNotification: Lists.newArrayList(page)) {
